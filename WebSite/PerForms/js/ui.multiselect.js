...
$.widget("ui.multiselect", {
  options: {
		...
		animated: 'fast',
        /*
            * (tag) switched slideDown and slideUp to fadeIn and fadeOut respectively.
        */
		show: 'fadeIn',
		hide: 'fadeOut',
	...
	_create: function() {
        ...
        /*
            * (tag) switched the order of the following 2 lines so the selectedContainer is at the right of the availableContainer
        ...
        */
		...
		/*
            * (tag) Added text-decoration: none to hide the underline on 'Add All' and 'Remove All'
        */
        this.selectedActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="count">0 item(s)</span><a href="#" class="remove-all" style="text-decoration: none;"><span style="font-size: 12px;font-weight: bold;"> - - </span></a></div>').appendTo(this.selectedContainer);
		this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="busy">...</span><input type="text" class="search ui-widget-content ui-corner-all"/><a href="#" class="add-all" style="text-decoration: none;"><span style="font-size: 12px; font-weight: bold;"> + + </span></a></div>').appendTo(this.availableContainer);
    ...
    **************************************/
	...
	_messages: function(type, msg, params) {
		this._trigger('messages', null, this._ui('message', type, ""));
	},
   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
	_setSelected: function(item, selected, noclone) {
        ...
        /*
            * (tag) Allows the server to run validation rules on the items.
        */
	...
	_applyItemState: function(item, selected) {
		if (selected) {
            ...
            /*
                * (tag) Clicking anywhere on the item should trigger the remove event
            */
            item.click(function () {
                item.find('a.action').click();
            });
            ...
            /*
                * (tag) Clicking anywhere on the item should trigger the add event
            */
            item.click(function () {
                item.find('a.action').click();
            });
...
$.extend($.ui.multiselect, {
	...
	locale: {
		addAll:'<span style="font-size: 12px; font-weight: bold;"> + + </span>',
		removeAll:'<span style="font-size: 12px;font-weight: bold;"> - - </span>',
